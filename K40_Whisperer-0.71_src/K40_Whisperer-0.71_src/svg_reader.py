...
class SVG_READER(inkex.Effect):
    def __init__(self):
        ...
        self.filled_polygons = [] # Store polygons for internal rasterization
        self.cut_lines = []
        self.eng_lines = []
        self.id_cnt = 0
    ...
    def process_shape(self, node, mat, group_stroke = None):
        ...
        if not fill_color:
            ...
            if style:
                for decl in style.split(';'):
                    parts = decl.split(':')
                    if len(parts) == 2 and parts[0].strip().lower() == 'fill':
                        fill_color = parts[1].strip()
        ...
        if changed or has_fill:
            ...
            for sub in p:
                ...
                if has_fill and len(polygon_points) > 2:
                    # Close the polygon point list
                    polygon_points.append(polygon_points[0])
                    self.filled_polygons.append({'points': polygon_points, 'color': fill_color, 'path_id': path_id})
        #####################################################
    ...
    def Make_PNG(self):
        if self.inkscape_exe != None:
            ...
            # Fallback to internal rasterizer if Inkscape is missing
            self.Make_PNG_Internal()
            return

    def Make_PNG_Internal(self):
        """Internal rasterizer using Pillow with Even-Odd fill rule support for holes."""
        try:
            from PIL import Image, ImageDraw, ImageColor, ImageChops
            
            width_px = int(abs(self.Xsize) / 25.4 * self.image_dpi)
            height_px = int(abs(self.Ysize) / 25.4 * self.image_dpi)
            if width_px <= 0 or height_px <= 0: width_px, height_px = 100, 100

            scale = self.image_dpi / 25.4
            self.raster_PIL = Image.new("L", (width_px, height_px), 255)
            h_mm_val = abs(self.Ysize)
            
            # Offset for the cropped area
            off_x, off_y = self.raster_offset_mm

            # Group polygons by path_id to handle holes (even-odd rule per path)
            paths = {}
            for poly in self.filled_polygons:
                pid = poly['path_id']
                if pid not in paths: paths[pid] = {'polys': [], 'color': poly['color']}
                paths[pid]['polys'].append(poly['points'])

            for pid in paths:
                # Create a 1-bit mask for this path
                path_mask = Image.new("1", (width_px, height_px), 0)
                
                for points in paths[pid]['polys']:
                    # Create a temporary mask for the sub-path
                    sub_mask = Image.new("1", (width_px, height_px), 0)
                    # Translate points relative to our new cropped origin (xmin, ymax)
                    scaled_points = [((p[0] - off_x) * scale, (off_y - p[1]) * scale) for p in points]
                    
                    # Draw sub-path on temp mask
                    draw_sub = ImageDraw.Draw(sub_mask)
                    draw_sub.polygon(scaled_points, fill=1)
                    
                    # XOR with path mask (Even-Odd rule)
                    path_mask = ImageChops.logical_xor(path_mask, sub_mask)
                
                # Render the final path mask onto the main image with its color
                color_str = paths[pid]['color']
                try:
                ...
                # Apply the mask: wherever path_mask is 1, paint with 'gray'
                self.raster_PIL.paste(gray, (0, 0), path_mask)
                
            print(f"K40 Independence: Rendered {len(paths)} complex paths ({width_px}x{height_px}px)")
        except Exception as e:
            print("Internal Rasterization Failed:", e)
            self.raster_PIL = Image.new("L", (100, 100), 255)
...
##        else:
##            # CorelDraw integration: Skip path conversion
##            return
##        try:
##            shutil.rmtree(tmp_dir)
##        except:
...
##        return tmpdir
    ...
    def convert_text2paths(self):
        ...
        if self.inkscape_exe != None:
            ...
            # CorelDraw integration: Skip text conversion
            return
        try:
            shutil.rmtree(tmp_dir)
        except:
    ...
    def make_paths(self, txt2paths=False ):
        ...
        #################################################
        ### Calculate actual bounds of the design     ###
        #################################################
        if self.lines or self.filled_polygons:
            all_x = [l[0] for l in self.lines] + [l[2] for l in self.lines]
            all_y = [l[1] for l in self.lines] + [l[3] for l in self.lines]
            for poly in self.filled_polygons:
            ...
            # Add a small margin (1mm) to avoid cropping the edges
            margin = 1.0
            xmin -= margin
            xmax += margin
            ymin -= margin
        ...
        # We need to inform the internal rasterizer about the translation
        self.raster_offset_mm = (xmin, ymax) 
