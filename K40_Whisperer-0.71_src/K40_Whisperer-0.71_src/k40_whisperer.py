...
from i18n import _
import theme
from machine_wizard import MachineWizard
try:
    import sv_ttk
...
title_text = _("K40 Whisperer") + " V" + version
...
if VERSION == 3:
    from tkinter import *
    from tkinter import ttk, scrolledtext, scrolledtext
...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.diag_crc = StringVar(value="CRC Errors: 0")
        self.diag_usb = StringVar(value="USB Timeouts: 0")
...
##        self.unsharp_t.set("3")
        ...
        # Perfis de Materiais BR
        self.material_profile = StringVar()
        self.MATERIALS_BR = {
            "Customizado": None,
            "MDF 3mm (Corte)": {"r_v": "100", "v_v": "20", "c_v": "8", "r_p": "0.5", "v_p": "0.3", "c_p": "0.8"},
            "MDF 3mm (Gravação)": {"r_v": "300", "v_v": "40", "c_v": "10", "r_p": "0.2", "v_p": "0.2", "c_p": "0.5"},
            "Acrílico 2mm": {"r_v": "250", "v_v": "30", "c_v": "12", "r_p": "0.3", "v_p": "0.3", "c_p": "0.6"},
            "Couro": {"r_v": "200", "v_v": "25", "c_v": "10", "r_p": "0.2", "v_p": "0.2", "c_p": "0.4"},
            "Papel/Papelão": {"r_v": "400", "v_v": "50", "c_v": "20", "r_p": "0.1", "v_p": "0.1", "c_p": "0.3"},
        }
        ...
        self.statusMessage.set(_("Welcome to K40 Whisperer"))
        ...
        self.diag_frame = Frame(self.master, bd=1, relief=SUNKEN)
        self.diag_frame.pack(side=BOTTOM, fill=X)
        self.diag_crc_label = Label(self.diag_frame, textvariable=self.diag_crc, fg="red", font=("Arial", 8))
        self.diag_crc_label.pack(side=RIGHT, padx=10)
        self.diag_usb_label = Label(self.diag_frame, textvariable=self.diag_usb, fg="orange", font=("Arial", 8))
        self.diag_usb_label.pack(side=RIGHT, padx=10)
        self.diag_title = Label(self.diag_frame, text="Diagnóstico de Hardware:", font=("Arial", 8, "bold"))
        self.diag_title.pack(side=LEFT, padx=5)
        ...
        # Materiais Dropdown
        self.Label_Materials = ttk.Label(self.master, text=_("Material Profile"), anchor=W)
        self.OptionMenu_Materials = ttk.Combobox(self.master, textvariable=self.material_profile,
                                                  values=list(self.MATERIALS_BR.keys()), state="readonly")
        self.OptionMenu_Materials.bind("<<ComboboxSelected>>", lambda e: self.Apply_Material_Profile(self.material_profile.get()))
        self.material_profile.set("Customizado")

        #Speed
        self.Label_Reng_feed_u = ttk.Label(self.master,textvariable=self.funits, anchor=W)
        self.Entry_Reng_feed   = ttk.Entry(self.master,width=15)
        ...
        self.Entry_Veng_feed   = ttk.Entry(self.master,width=15)
        self.Entry_Veng_feed.configure(textvariable=self.Veng_feed,justify='center')
        ...
        self.Entry_Vcut_feed   = ttk.Entry(self.master,width=15)
        self.Entry_Vcut_feed.configure(textvariable=self.Vcut_feed,justify='center')
        ...
        self.Label_power_u = ttk.Label(self.master,text=_("Power\nFraction"), anchor=CENTER)
        self.Label_time_u  = ttk.Label(self.master,text=_("Time\nmSec"), anchor=CENTER)
        self.Label_power2_u= ttk.Label(self.master,text=_("Power\nFraction"), anchor=CENTER)

        self.Entry_Reng_power   = ttk.Entry(self.master,width=15)
        ...
        self.Entry_Veng_power   = ttk.Entry(self.master,width=15)
        self.Entry_Veng_power.configure(textvariable=self.Veng_power,justify='center')
        trace_variable(self.Veng_power, self.Entry_Veng_power_Callback)

        self.Entry_Vcut_power   = ttk.Entry(self.master,width=15)
        self.Entry_Vcut_power.configure(textvariable=self.Vcut_power,justify='center')
        trace_variable(self.Vcut_power, self.Entry_Vcut_power_Callback)

        self.Entry_Gcode_power   = ttk.Entry(self.master,width=15)
        self.Entry_Gcode_power.configure(textvariable=self.Gcode_power,justify='center')
        trace_variable(self.Gcode_power, self.Entry_Gcode_power_Callback)


        ### Test Fire ###
        self.Test_Button  = ttk.Button(self.master,text=_("Test Fire Laser"), command=self.Test_Fire)
        self.Label_Test_time_u = ttk.Label(self.master,text=_("ms"), anchor=W)
        self.Entry_Test_time   = ttk.Entry(self.master,width=15)
        ...
        self.Entry_Test_power   = ttk.Entry(self.master,width=15)
        ...
        ##################
        # Buttons de operação
        self.Reng_Button  = ttk.Button(self.master,text=_("Raster Engrave"), command=self.Raster_Eng)
        self.Veng_Button  = ttk.Button(self.master,text=_("Vector Engrave"), command=self.Vector_Eng)
        self.Vcut_Button  = ttk.Button(self.master,text=_("Vector Cut"),     command=self.Vector_Cut)
        self.Grun_Button  = ttk.Button(self.master,text=_("Run G-Code"),     command=self.Gcode_Cut)

        self.Reng_Veng_Button      = ttk.Button(self.master,text=_("Raster and\nVector Engrave"), command=self.Raster_Vector_Eng)
        self.Veng_Vcut_Button      = ttk.Button(self.master,text=_("Vector Engrave\nand Cut"),    command=self.Vector_Eng_Cut)
        self.Reng_Veng_Vcut_Button = ttk.Button(self.master,text=_("Raster Engrave\nVector Engrave\nand\nVector Cut"), command=self.Raster_Vector_Cut)

        self.Label_Position_Control = ttk.Label(self.master,text=_("Position Controls:"), anchor=W)

        self.Initialize_Button = ttk.Button(self.master,text=_("Initialize Laser Cutter"), command=self.Initialize_Laser, style="Accent.TButton")
        self.USB_Indicator = Label(self.master, text="🔴 DESCONECTADO", fg="red", font=("Arial", 9, "bold"))

        self.Open_Button   = ttk.Button(self.master,text=_("Open\nDesign File"),   command=self.menu_File_Open_Design)
        self.Reload_Button = ttk.Button(self.master,text=_("Reload\nDesign File"), command=self.menu_Reload_Design)

        self.Home_Button   = ttk.Button(self.master,text=_("Home"),        command=self.Home)
        self.UnLock_Button = ttk.Button(self.master,text=_("Unlock Rail"), command=self.Unlock)
        self.Stop_Button   = ttk.Button(self.master,text=_("Pause/Stop"),  command=self.Stop)
        ...
        try:
            ...
            self.Right_Button = ttk.Button(self.master,image=self.right_image, command=self.Move_Right)
            self.Left_Button  = ttk.Button(self.master,image=self.left_image,  command=self.Move_Left)
            self.Up_Button    = ttk.Button(self.master,image=self.up_image,    command=self.Move_Up)
            self.Down_Button  = ttk.Button(self.master,image=self.down_image,  command=self.Move_Down)
            ...
            self.UL_Button = ttk.Button(self.master,image=self.UL_image, command=self.Move_UL)
            self.UR_Button = ttk.Button(self.master,image=self.UR_image, command=self.Move_UR)
            self.LR_Button = ttk.Button(self.master,image=self.LR_image, command=self.Move_LR)
            self.LL_Button = ttk.Button(self.master,image=self.LL_image, command=self.Move_LL)
            self.CC_Button = ttk.Button(self.master,image=self.CC_image, command=self.Move_CC)

        except:
            self.Right_Button = ttk.Button(self.master,text=">", command=self.Move_Right)
            self.Left_Button  = ttk.Button(self.master,text="<", command=self.Move_Left)
            self.Up_Button    = ttk.Button(self.master,text="^", command=self.Move_Up)
            self.Down_Button  = ttk.Button(self.master,text="v", command=self.Move_Down)

            self.UL_Button = ttk.Button(self.master,text=" ", command=self.Move_UL)
            self.UR_Button = ttk.Button(self.master,text=" ", command=self.Move_UR)
            self.LR_Button = ttk.Button(self.master,text=" ", command=self.Move_LR)
            self.LL_Button = ttk.Button(self.master,text=" ", command=self.Move_LL)
            self.CC_Button = ttk.Button(self.master,text=" ", command=self.Move_CC)

        self.Label_Step   = ttk.Label(self.master,text=_("Jog Step"), anchor=CENTER)
        self.Label_Step_u = ttk.Label(self.master,textvariable=self.units, anchor=W)
        self.Entry_Step   = ttk.Entry(self.master,width=15)
        self.Entry_Step.configure(textvariable=self.jog_step, justify='center')
        trace_variable(self.jog_step, self.Entry_Step_Callback)

        ###########################################################################
        self.GoTo_Button = ttk.Button(self.master,text=_("Move To"), command=self.GoTo)

        self.Entry_GoToX = ttk.Entry(self.master,width=15,justify='center')
        self.Entry_GoToX.configure(textvariable=self.gotoX)
        trace_variable(self.gotoX, self.Entry_GoToX_Callback)
        self.Entry_GoToY = ttk.Entry(self.master,width=15,justify='center')
        ...
        # Advanced Column
        self.separator_vert = ttk.Separator(self.master, orient='vertical')
        self.Label_Advanced_column = ttk.Label(self.master,text=_("Advanced Settings"),anchor=CENTER)
        self.separator_adv  = ttk.Separator(self.master, orient='horizontal')

        self.Label_Halftone_adv = ttk.Label(self.master,text=_("Halftone (Dither)"))
        self.Checkbutton_Halftone_adv = ttk.Checkbutton(self.master,text=" ")
        self.Checkbutton_Halftone_adv.configure(variable=self.halftone)
        trace_variable(self.halftone, self.View_Refresh_and_Reset_RasterPath)

        self.Label_Negate_adv = ttk.Label(self.master,text=_("Invert Raster Color"))
        self.Checkbutton_Negate_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.separator_adv2 = ttk.Separator(self.master, orient='horizontal')

        self.Label_Mirror_adv = ttk.Label(self.master,text=_("Mirror Design"))
        self.Checkbutton_Mirror_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.Label_Rotate_adv = ttk.Label(self.master,text=_("Rotate Design"))
        self.Checkbutton_Rotate_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.separator_adv3 = ttk.Separator(self.master, orient='horizontal')

        self.Label_inputCSYS_adv = ttk.Label(self.master,text=_("Use Input CSYS"))
        self.Checkbutton_inputCSYS_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.Label_Inside_First_adv = ttk.Label(self.master,text=_("Cut Inside First"))
        self.Checkbutton_Inside_First_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.Label_Rotary_Enable_adv = ttk.Label(self.master,text=_("Use Rotary Settings"))
        self.Checkbutton_Rotary_Enable_adv = ttk.Checkbutton(self.master,text="")
        ...
        #####
        self.separator_comb = ttk.Separator(self.master, orient='horizontal')

        self.Label_Comb_Engrave_adv = ttk.Label(self.master,text=_("Group Engrave Tasks"))
        self.Checkbutton_Comb_Engrave_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.Label_Comb_Vector_adv = ttk.Label(self.master,text=_("Group Vector Tasks"))
        self.Checkbutton_Comb_Vector_adv = ttk.Checkbutton(self.master,text=" ")
        ...
        self.Label_Reng_passes = ttk.Label(self.master,text=_("Raster Eng. Passes"))
        self.Entry_Reng_passes = ttk.Entry(self.master,width=15)
        ...
        self.Label_Veng_passes = ttk.Label(self.master,text=_("Vector Eng. Passes"))
        self.Entry_Veng_passes = ttk.Entry(self.master,width=15)
        self.Entry_Veng_passes.configure(textvariable=self.Veng_passes,justify='center')
        trace_variable(self.Veng_passes, self.Entry_Veng_passes_Callback)

        self.Label_Vcut_passes = ttk.Label(self.master,text=_("Vector Cut Passes"))
        self.Entry_Vcut_passes = ttk.Entry(self.master,width=15)
        self.Entry_Vcut_passes.configure(textvariable=self.Vcut_passes,justify='center')
        trace_variable(self.Vcut_passes, self.Entry_Vcut_passes_Callback)

        self.Label_Gcde_passes = ttk.Label(self.master,text=_("G-Code Passes"))
        self.Entry_Gcde_passes = ttk.Entry(self.master,width=15)
        ...
        self.Hide_Adv_Button = ttk.Button(self.master,text=_("Hide Advanced"), command=self.Hide_Advanced)

        # End Right Column #
        self.calc_button = ttk.Button(self.master,text=_("Calculate Raster Time"), command=self.menu_Calc_Raster_Time)

        #GEN Setting Window Entry initializations
        self.Entry_Sspeed    = ttk.Entry()
        self.Entry_BoxGap    = ttk.Entry()
        self.Entry_ContAngle = ttk.Entry()
        ...
        top_File.add("command", label = _("Save Settings File"), command = self.menu_File_Save)
        top_File.add("command", label = _("Read Settings File"), command = self.menu_File_Open_Settings_File)

        top_File.add_separator()
        top_File.add("command", label = _("Open Design (SVG/DXF/G-Code)")  , command = self.menu_File_Open_Design)
        top_File.add("command", label = _("Reload Design")          , command = self.menu_Reload_Design)

        top_File.add_separator()    
        top_File.add("command", label = _("Send EGV File to Laser")             , command = self.menu_File_Open_EGV)

        SaveEGVmenu = Menu(self.master, relief = "raised", bd=2, tearoff=0)
        top_File.add_cascade(label=_("Save EGV File"), menu=SaveEGVmenu)        
        SaveEGVmenu.add("command", label = _("Raster Engrave")     , command = self.menu_File_Raster_Engrave)
        SaveEGVmenu.add("command", label = _("Vector Engrave")     , command = self.menu_File_Vector_Engrave)
        SaveEGVmenu.add("command", label = _("Vector Cut")         , command = self.menu_File_Vector_Cut)
        SaveEGVmenu.add_separator()   
        SaveEGVmenu.add("command", label = _("Raster and Vector Engrave")             , command = self.menu_File_Raster_Vector_Engrave)
        SaveEGVmenu.add("command", label = _("Vector Engrave and Cut")                , command = self.menu_File_Vector_Engrave_Cut)
        SaveEGVmenu.add("command", label = _("Raster, Vector Engrave and Vector Cut") , command = self.menu_File_Raster_Vector_Cut)
        
    
        top_File.add_separator()
        top_File.add("command", label = _("Exit"), command = self.menu_File_Quit)
        
        self.menuBar.add("cascade", label=_("File"), menu=top_File)
        ...
        top_View.add("command", label = _("Refresh   <F5>"), command = self.menu_View_Refresh)
        top_View.add_separator()
        top_View.add_checkbutton(label = _("Show Raster Image")  ,  variable=self.include_Reng ,command= self.menu_View_Refresh)
        if DEBUG:
            top_View.add_checkbutton(label = _("Show Raster Paths") ,variable=self.include_Rpth ,command= self.menu_View_Refresh)
        
        top_View.add_checkbutton(label = _("Show Vector Engrave"),   variable=self.include_Veng ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = _("Show Vector Cut")    ,   variable=self.include_Vcut ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = _("Show G-Code Paths")  ,   variable=self.include_Gcde ,command= self.menu_View_Refresh)
        top_View.add_separator()
        top_View.add_checkbutton(label = _("Show Time Estimates"),   variable=self.include_Time ,command= self.menu_View_Refresh)
        top_View.add_checkbutton(label = _("Zoom to Design Size"),   variable=self.zoom2image   ,command= self.menu_View_Refresh)
        ...
        self.menuBar.add("cascade", label=_("View"), menu=top_View)

        top_Tools = Menu(self.menuBar, tearoff=0)
        self.menuBar.add("cascade", label=_("Tools"), menu=top_Tools)
        USBmenu = Menu(self.master, relief = "raised", bd=2, tearoff=0)
          

        top_Tools.add("command", label = _("Calculate Raster Time"), command = self.menu_Calc_Raster_Time)
        top_Tools.add("command", label = _("Trace Design Boundary <Ctrl-t>"), command = self.TRACE_Settings_Window)
        top_Tools.add_separator()
        top_Tools.add("command", label = _("Initialize Laser <Ctrl-i>"), command = self.Initialize_Laser)
        top_Tools.add("command", label = _("Unfreeze Laser <Ctrl-f>")  , command = self.Unfreeze_Laser)
        top_Tools.add_cascade(label=_("USB"), menu=USBmenu)
        USBmenu.add("command", label = _("Reset USB"), command = self.Reset)
        USBmenu.add("command", label = _("Release USB"), command = self.Release_USB)
        ...
        top_Settings.add("command", label = _("General Settings <F2>"), command = self.GEN_Settings_Window)
        top_Settings.add("command", label = _("Raster Settings <F3>"),  command = self.RASTER_Settings_Window)
        top_Settings.add("command", label = _("Rotary Settings <F4>"),  command = self.ROTARY_Settings_Window)
        top_Settings.add_separator()
        top_Settings.add_checkbutton(label = _("Advanced Settings <F6>"), variable=self.advanced ,command= self.menu_View_Refresh)
        top_Settings.add_separator()
        top_Settings.add("command", label = _("⚙  Assistente de Máquina"), command = self.Open_Machine_Wizard)
        top_Settings.add("command", label = _("☀  Alternar Tema Claro/Escuro"), command = self.Toggle_Theme)

        self.menuBar.add("cascade", label=_("Settings"), menu=top_Settings)
        
        top_Help = Menu(self.menuBar, tearoff=0)
        top_Help.add("command", label = _("About (e-mail)"), command = self.menu_Help_About)
        top_Help.add("command", label = _("K40 Whisperer Web Page"), command = self.menu_Help_Web)
        top_Help.add("command", label = _("Manual (Web Page)"), command = self.menu_Help_Manual)
        self.menuBar.add("cascade", label=_("Help"), menu=top_Help)
    ...
    def entry_set(self, val2, calc_flag=0, new=0):
        if calc_flag == 0 and new==0:
            try:
                ...
                self.statusMessage.set(_(" Recalculation required."))
            except:
                pass
        elif calc_flag == 3:
            try:
                ...
                self.statusMessage.set(_(" Value should be a number. "))
            except:
                pass
        elif calc_flag == 2:
            try:
...
################################################################################
    def Write_Config_File(self, event):
        ...
        try:
            ...
            self.statusMessage.set(_("Unable to open file for writing: %s") %(configname_full))
            self.statusbar.configure( bg = 'red' )
            return
        ...
        fout.close
        self.statusMessage.set(_("Configuration File Saved: %s") %(configname_full))
        self.statusbar.configure( bg = 'white' )
        try:
            win_id.deiconify()
        except:
...
##        header.append('(k40_whisperer_set unsharp_t     %s )'  %( self.unsharp_t.get()      ))
    ...
    def Quit_Click(self, event):
        self.statusMessage.set(_("Exiting!"))
        self.Release_USB
        root.destroy()
...
##        self.refreshTime()
    ...
    def Entry_Reng_feed_Check(self):
        try:
            ...
            if  value < low_limit:
                self.statusMessage.set(_(" Feed Rate should be greater than or equal to %f ") %(low_limit))
    ...
    def Entry_Reng_power_Check(self):
        try:
            ...
            if  value < 0 or value > high_limit:
                self.statusMessage.set(_(" Power fraction should be between 0.00 and 1.00 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.refreshTime()
    ...
    def Entry_Test_time_Check(self):
        try:
            ...
            high_limit = 9999
            if  value < 0 or value > high_limit:
                self.statusMessage.set(_(" Time should be between 0 and 9999 ms "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.refreshTime()
    ...
    def Entry_Step_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Step should be greater than 0.0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_GoToX_Check(self):
        try:
            ...
            if  (value < 0.0) and (not self.HomeUR.get()):
                self.statusMessage.set(_(" Value should be greater than 0.0 "))
                return 2 # Value is invalid number
            elif (value > 0.0) and self.HomeUR.get():
                self.statusMessage.set(_(" Value should be less than 0.0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Rstep_Check(self):
        try:
            ...
            if  value <= 0 or value > 63:
                self.statusMessage.set(_(" Step should be between 0.001 and 0.063 in"))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
...
##        self.entry_set(self.Entry_Unsharp_Threshold, self.Entry_Unsharp_Threshold_Check(), new=1)
    ...
    def Entry_Ink_Timeout_Check(self):
        try:
            ...
            if  value < 0.0:
                self.statusMessage.set(_(" Timeout should be 0 or greater"))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Timeout_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Timeout should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_N_Timeouts_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" N_Timeouts should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_N_EGV_Passes_Check(self):
        try:
            ...
            if  value < 1:
                self.statusMessage.set(_(" EGV passes should be 1 or higher"))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Laser_Area_Width_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Width should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Max_Power_Check(self):
        try:
            ...
            if  value < 0.0 or value > 100.0:
                self.statusMessage.set(_(" Max power should be between 0% and 100% "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Laser_Area_Height_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Height should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Laser_X_Scale_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" X scale factor should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.Reset_RasterPath_and_Update_Time()
    ...
    def Entry_Laser_Y_Scale_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Y scale factor should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.Reset_RasterPath_and_Update_Time()
    ...
    def Entry_Laser_R_Scale_Check(self):
        try:
            ...
            if  value <= 0.0:
                self.statusMessage.set(_(" Rotary scale factor should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.Reset_RasterPath_and_Update_Time()
    ...
    def Entry_Laser_Rapid_Feed_Check(self):
        try:
            ...
            if  value !=0 and value < low_limit:
                self.statusMessage.set(_(" Rapid feed should be greater than or equal to %f (or 0 for default speed) ") %(low_limit))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        return 0         # Value is a valid number
    ...
    def Entry_Reng_passes_Check(self):
        try:
            ...
            if  value < 1:
                self.statusMessage.set(_(" Number of passes should be greater than 0 "))
                return 2 # Value is invalid number
        except:
            return 3     # Value not a number
        self.refreshTime()
    ...
    def menu_Reload_Design(self,event=None):
        ...
        if ( os.path.isfile(file_full) ):
            ...
            self.statusMessage.set(_("file not found: %s") %(os.path.basename(file_full)) )
            self.statusbar.configure( bg = 'red' ) 
            return
    ...
    def Sanitize_SVG(self, filename):
        """Pre-process SVG to remove BOM and normalize encoding for inkex."""
        try:
            ...
            if data.startswith(b'\xef\xbb\xbf'):
                data = data[3:]
            # Normalize UTF-16 to UTF-8 (Corel sometimes does this)
            if data.startswith(b'\xff\xfe') or data.startswith(b'\xfe\xff'):
                data = data.decode('utf-16').encode('utf-8')
            
            # Strip problematic XML processing instructions that inkex 0.71 hates
            text = data.decode('utf-8', errors='ignore')
            # Remove standalone="no" or similar if causing issues
    ...
    def Load_Design_File(self, fileselect):

        Name, fileExtension = os.path.splitext(fileselect)
        self.update_gui(_("Opening '%s'") % fileselect )
        TYPE=fileExtension.upper()
        if TYPE=='.DXF':
            self.Open_DXF(fileselect)
        elif TYPE=='.SVG':
            self.Sanitize_SVG(fileselect)
    ...
    def menu_File_Open_Design(self,event=None):
        ...
        design_types = (_("Design Files"), ("*.svg","*.dxf"))
        gcode_types  = (_("G-Code Files"), ("*.ngc","*.gcode","*.g","*.tap"))
        ...
        fileselect = askopenfilename(filetypes=[default_types,
                                            (_("G-Code Files "), ("*.ngc","*.gcode","*.g","*.tap")),\
                                            (_("DXF Files "),"*.dxf"),\
                                            (_("SVG Files "),"*.svg"),\
                                            (_("All Files "),"*"),\
                                            (_("Design Files "), ("*.svg","*.dxf"))],\
                                            initialdir=init_dir)

        if fileselect == () or (not os.path.isfile(fileselect)):
            return
            
        self.Load_Design_File(fileselect)

    def Apply_Material_Profile(self, selection):
        profile = self.MATERIALS_BR.get(selection)
        if profile:
            self.Reng_feed.set(profile["r_v"])
            self.Veng_feed.set(profile["v_v"])
            self.Vcut_feed.set(profile["c_v"])
            self.Reng_power.set(profile["r_p"])
            self.Veng_power.set(profile["v_p"])
            self.Vcut_power.set(profile["c_p"])
            self.statusMessage.set(_("Material profile applied: %s") % selection)
    ...
    def menu_File_save_EGV(self,operation_type=None,default_name="out.EGV"):
        ...
        if filename != '' and filename != ():
            ...
            if operation_type.find("Raster_Eng") > -1:
                ...
                self.statusbar.configure( bg = 'yellow' )
                self.statusMessage.set(_("No raster data to engrave"))
    ...
    def Open_SVG(self,filemname):
        ...
        try:
            try:
                ...
                svg_reader.set_inkscape_path(self.inkscape_path.get())
                svg_reader.timout = int(float( self.ink_timeout.get())*60.0) 
                self.statusMessage.set(_("Converting TEXT to PATHS."))
                self.master.update()
                svg_reader.parse_svg(self.SVG_FILE)
            ...
            self.statusMessage.set(_("Unable To open SVG File: %s") %(filemname))
            debug_message(traceback.format_exc())
            return
        ...
        # --- NORMALIZAÇÃO PARA CORELDRAW (BRIDGE) ---
        # Se for o arquivo de ponte do Corel, alinha automaticamente ao (0,0) na bolinha
        if "bridge" in filemname.lower() and filemname.lower().endswith(".svg"):
            design_all_x = []
            design_all_y = []
            if self.VcutData.ecoords:
                design_all_x.extend([p[0] for p in self.VcutData.ecoords])
                design_all_y.extend([p[1] for p in self.VcutData.ecoords])
            if self.VengData.ecoords:
                design_all_x.extend([p[0] for p in self.VengData.ecoords])
                design_all_y.extend([p[1] for p in self.VengData.ecoords])
            
            if design_all_x and design_all_y:
                min_x = min(design_all_x)
                min_y = min(design_all_y)
                
                # Shift coordinates
                for p in self.VcutData.ecoords:
                    p[0] -= min_x
                    p[1] -= min_y
                for p in self.VengData.ecoords:
                    p[0] -= min_x
                    p[1] -= min_y
                    
                # Recalcular limites dos dados
                self.VcutData.computeEcoordsLen()
                self.VengData.computeEcoordsLen()
                
                # Ajustar limites do design para caber exatamente no conteúdo (iniciando em 0,0)
                new_xmax = max(self.VcutData.bounds[1], self.VengData.bounds[1])
                new_ymax = max(self.VcutData.bounds[3], self.VengData.bounds[3])
                self.Design_bounds = (0.0, new_xmax, 0.0, new_ymax)
...
##                    image_temp = image_temp.filter(filter)
                ...
                for i_step in range(0,im_height_mils,Raster_step):
                    ...
                    if (stamp != timestamp):
                        ...
                        self.statusMessage.set(_("Creating Scan Lines: %.1f %%") %( (100.0*i)/him ) )
                        self.master.update()
                    if self.stop[0]==True:
    ...
    def convert_halftoning(self,image):
        ...
        if w > 0:
            ...
            for y in range(1, y_lim):
                ...
                if (stamp != timestamp):
                    timestamp=stamp #interlock
                    self.statusMessage.set(_("Adjusting Image Darkness: %.1f %%") %( (100.0*y)/y_lim ) )
                    self.master.update()
                for x in range(1, x_lim):
                    pixel[x, y] = val_map[ pixel[x, y] ]

        self.statusMessage.set(_("Creating Halftone Image.") )
        self.master.update()
...
##            fout.close
    ...
    ##                     self.unsharp_t.set(line[line.find("unsharp_t"):].split()[1])
        ...
        if init_file != "None":
            if ( os.path.isfile(self.DESIGN_FILE) ):
                ...
                self.statusMessage.set(_("Image file not found: %s ") %(self.DESIGN_FILE))
    ...
    def menu_File_Save(self):
        ...
        if filename != '' and filename != ():
            try:
                ...
                self.statusMessage.set(_("Unable to open file for writing: %s") %(filename))
                self.statusbar.configure( bg = 'red' )
                return
            ...
            for line in settings_data:
                try:
                    ...
                    debug_message(traceback.format_exc())
            fout.close
            self.statusMessage.set(_("File Saved: %s") %(filename))
            self.statusbar.configure( bg = 'white' )
    ...
    def update_gui(self, message=None, bgcolor='white'):
        ...
        if self.k40:
            self.diag_crc.set("CRC Errors: %d" % self.k40.total_crc_errors)
            self.diag_usb.set("USB Timeouts: %d" % self.k40.total_usb_timeouts)
    ...
    def Vector_Cut(self, output_filename=None):
        ...
        if self.VcutData.ecoords!=[]:
            ...
            self.statusbar.configure( bg = 'yellow' )
            self.statusMessage.set(_("No vector data to cut"))
        self.Finish_Job()
    ...
    def Vector_Eng(self, output_filename=None):
        ...
        if self.VengData.ecoords!=[]:
            ...
            self.statusbar.configure( bg = 'yellow' )
            self.statusMessage.set(_("No vector data to engrave"))
        self.Finish_Job()
    ...
    def Trace_Eng(self, output_filename=None):
        ...
        if self.trace_coords!=[]:
            ...
            self.statusbar.configure( bg = 'yellow' )
            self.statusMessage.set(_("No trace data to follow"))
        self.Finish_Job()
    ...
    def Raster_Vector_Eng(self, output_filename=None):
        ...
        try:
            ...
            if self.RengData.ecoords!=[] or self.VengData.ecoords!=[]:
                ...
                self.statusbar.configure( bg = 'yellow' )
                self.statusMessage.set(_("No data to engrave"))
        except Exception as e:
    ...
    def Vector_Eng_Cut(self, output_filename=None):
        ...
        if self.VcutData.ecoords!=[] or self.VengData.ecoords!=[]:
            ...
            self.statusbar.configure( bg = 'yellow' )
            self.statusMessage.set(_("No vector data."))
        self.Finish_Job()
    ...
    def Raster_Vector_Cut(self, output_filename=None):
        ...
        try:
            ...
            if self.RengData.ecoords!=[] or self.VengData.ecoords!=[] or self.VcutData.ecoords!=[]:
                ...
                self.statusbar.configure( bg = 'yellow' )
                self.statusMessage.set(_("No data to engrave/cut"))
        except Exception as e:
            msg1 = "Preparing Data Stopped: "
            msg2 = "%s" %(e)
            self.statusMessage.set((msg1+msg2).split("\n")[0] )
    ...
    def Gcode_Cut(self, output_filename=None):
        ...
        if self.GcodeData.ecoords!=[]:
            ...
            self.statusbar.configure( bg = 'yellow' )
            self.statusMessage.set(_("No g-code data to cut"))
        self.Finish_Job()
    ...
    def send_data(self,operation_type=None, output_filename=None):
        ...
        if self.k40 == None and output_filename == None:
            self.statusMessage.set(_("Laser Cutter is not Initialized..."))
            self.statusbar.configure( bg = 'red' ) 
            return
        try:
            ...
            if (operation_type.find("Vector_Cut") > -1) and  (self.VcutData.ecoords!=[]):
                ...
                self.statusMessage.set(_("Vector Cut: Determining Cut Order...."))
                self.master.update()
...
##                    plt.plot(X,Y)
                ...
                self.statusMessage.set(_("Generating EGV data..."))
                self.master.update()
            ...
            if (operation_type.find("Vector_Eng") > -1) and  (self.VengData.ecoords!=[]):
                ...
                self.statusMessage.set(_("Vector Engrave: Determining Cut Order...."))
                self.master.update()
            ...
            if self.display_power:
                ...
                if Raster_Eng_data!=[]:
                    data=[]
                    data.append(ord("I"))
                    for k in range(int(float(self.Reng_passes.get()))):
                        if len(data)> 4:
                            data[-4]=ord("@")
                    ...
                    if power_fraction > 1: power_fraction=0
                    power = power_fraction*max_power
                    self.send_egv_data(data, 1, power_level=power, fast_mode=True)
                    self.menu_View_Refresh()

                if Vector_Eng_data!=[]:
                    data=[]
                    data.append(ord("I"))
                    for k in range(int(float(self.Veng_passes.get()))):
                        if len(data)> 4:
                            data[-4]=ord("@")
                ...
                if Vector_Cut_data!=[]:
                    data=[]
                    data.append(ord("I"))
                    for k in range(int(float(self.Vcut_passes.get()))):
                        if len(data)> 4:
                            data[-4]=ord("@")
                ...
                if G_code_Cut_data!=[]:
                    data=[]
                    data.append(ord("I"))
                    for k in range(int(float(self.Gcde_passes.get()))):
                        if len(data)> 4:
                            data[-4]=ord("@")
                ...
                if output_filename != None:
                    ...
                    # Se houver dados de Raster, ativamos o fast_mode para evitar engasgos
                    is_raster = Raster_Eng_data != []
                    self.send_egv_data(data, 1, power_level=None, fast_mode=is_raster)
                    self.menu_View_Refresh()
                #########################################################
    ...
    def send_egv_data(self,data,num_passes=1,power_level=None,fast_mode=False):
        if self.k40 is None:
            self.statusMessage.set(_("Laser is not initialized."))
            self.statusbar.configure( bg = 'yellow' )
            return

        time_start = time()

        # ── Máquina CO2 K40: comportamento original ──
        ...
        self.k40.send_data(data, self.update_gui, self.stop, num_passes,
                           pre_process_CRC, wait_for_laser=self.wait.get(), fast_mode=fast_mode)
    ...
    def write_egv_to_file(self,data,fname):
        ...
        fout.close
        self.menu_View_Refresh()
        self.statusMessage.set(_("Data saved to: %s") %(fname))
    ...
    def Reset(self):
        if self.k40 != None:
            try:
                self.k40.reset_usb()
                self.statusMessage.set(_("USB Reset Succeeded"))
            except:
                debug_message(traceback.format_exc())
                pass
    ...
    def Release_USB(self):
        if self.k40 != None:
            try:
                ...
                self.statusMessage.set(_("USB Release Succeeded"))
            except:
                debug_message(traceback.format_exc())
                pass
            self.k40=None
    ...
    def log_to_integrated_console(self, msg):
        if hasattr(self, "integrated_log"):
            self.integrated_log.config(state='normal')
            self.integrated_log.insert('end', msg)
            self.integrated_log.see('end')
            self.integrated_log.config(state='disabled')
    ...
    def Initialize_Laser(self, event=None):
        ...
        try:
            ...
            self.statusMessage.set(_("USB Error: %s") % error_text)
            self.statusbar.configure(bg='red')
            self.k40 = None
            debug_message(traceback.format_exc())
        except:
            self.statusMessage.set(_("Unknown USB Error"))
            self.statusbar.configure(bg='red')
            self.k40 = None
            debug_message(traceback.format_exc())

    ##########################################################################
    #                   Theme Toggle & Machine Wizard                        #
    ##########################################################################

    def Toggle_Theme(self):
        """Alterna entre tema claro e escuro (sv-ttk)."""
        if SV_TTK_AVAILABLE:
            import sv_ttk
            ...
            self.statusMessage.set("%s  Tema alterado para: %s" % (icon, new_theme))
            self.statusbar.configure(bg='white')
        else:
            self.statusMessage.set("sv-ttk não está instalado.")

    def Open_Machine_Wizard(self):
        """Abre o assistente de configuração de máquina."""
        current = {
            "board_name"        : self.board_name.get(),
            "com_port"          : self.grbl_port.get(),
            "baud_rate"         : int(self.grbl_baud.get() or 115200),
            "laser_area_width"  : float(self.LaserXsize.get() or 300),
            "laser_area_height" : float(self.LaserYsize.get() or 200),
        }
        wiz = MachineWizard(self.master, current_config=current)
        self.master.wait_window(wiz)
        if wiz.result:
            res = wiz.result
            self.board_name.set(res["board_name"])
            self.grbl_port.set(res["com_port"])
    ...
    def Unfreeze_Laser(self,event=None):
        ...
        if self.k40 != None:
            try:
                self.k40.unfreeze()
                self.statusMessage.set(_("Unfreeze Complete"))
                self.statusbar.configure( bg = 'white' )
            except:
                pass
    ...
    def Unlock(self,event=None):
        ...
        if self.k40 != None:
            try:
                ...
                self.statusMessage.set(_("Rail Unlock Succeeded"))
                self.statusbar.configure( bg = 'white' )
            except:
                self.statusMessage.set(_("Rail Unlock Failed."))
                self.statusbar.configure( bg = 'red' )
                debug_message(traceback.format_exc())
                pass
    ...
    def Master_Configure(self, event, update=0):
        ...
        if abs(self.w-w)>10 or abs(self.h-h)>10 or update==1:
            ...
            if True:                
                ...
                Yloc=10
                self.Initialize_Button.place(x=12, y=Yloc, width=210, height=42)
                self.USB_Indicator.place(x=12, y=Yloc+43, width=210, height=15)
                Yloc=Yloc+58

                # Materiais Dropdown
                self.Label_Materials.place(x=x_label_L, y=Yloc, width=w_label, height=21)
                self.OptionMenu_Materials.place(x=x_entry_L, y=Yloc, width=w_entry+w_units+4, height=23)
                Yloc=Yloc+33

                self.Open_Button.place (x=12, y=Yloc, width=104, height=42)
                self.Reload_Button.place(x=12+104, y=Yloc, width=104, height=42)
                if h>=self.pi_mode_height:
                    ...
                    Yloc=Yloc+28
                    self.Home_Button.place (x=12, y=Yloc, width=104, height=30)
                    self.UnLock_Button.place(x=12+104, y=Yloc, width=104, height=30)
                ...
                BUinit = self.h-70
                Yloc = BUinit
                self.Stop_Button.place(x=12, y=Yloc, width=210, height=34)
                ...
                if self.GcodeData.ecoords == []:
                    ...
                    Yloc=Yloc-34
                    self.Vcut_Button.place      (x=12, y=Yloc, width=210, height=28)
                    ...
                    Yloc=Yloc-34
                    self.Veng_Button.place  (x=12, y=Yloc, width=210, height=28)
                    ...
                    Yloc=Yloc-34
                    self.Reng_Button.place  (x=12, y=Yloc, width=210, height=28)
                    ...
                    if self.comb_vector.get() or self.comb_engrave.get():
                        ...
                        if self.comb_engrave.get():
                            if self.comb_vector.get():
                                self.Reng_Veng_Vcut_Button.place(x=12, y=Y_Reng, width=210, height=28*3+14)
                            else:
                                self.Reng_Veng_Button.place(x=12, y=Y_Reng, width=210, height=28*2+7)
                        elif self.comb_vector.get():
                            self.Veng_Vcut_Button.place(x=12, y=Y_Veng, width=210, height=28*2+7)
                    ...
                    if (self.display_test and h>=self.pi_mode_height):
                        Yloc=Yloc-34
                        self.Test_Button.place  (x=12, y=Yloc, width=210, height=28)
                    ...
                    Yloc=Yloc-34
                    self.Grun_Button.place  (x=12, y=Yloc, width=210, height=28)
                    if (self.display_power):
                        Yloc=Yloc-30
                        if (self.display_power):
...
##                            plot_im = plot_im.filter(filter)
    ...
    def GEN_Settings_Window(self):
        ...
        gen_settings.title(_("General Settings"))
        gen_settings.iconname(_("General Settings"))
        ...
        #Radio Button
        D_Yloc=D_Yloc+D_dY
        self.Label_Units = Label(gen_settings,text=_("Units"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_init_home = Label(gen_settings,text=_("Home Upon Initialize"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_post_home = Label(gen_settings,text=_("After Job Finishes:"))
        ...
        self.Checkbutton_post_home = Checkbutton(gen_settings,text=_("Unlock Rail"), anchor=W)
        ...
        self.Checkbutton_post_beep = Checkbutton(gen_settings,text=_("Beep"), anchor=W)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Checkbutton_post_disp = Checkbutton(gen_settings,text=_("Popup Report"), anchor=W)
        ...
        self.Checkbutton_post_exec = Checkbutton(gen_settings,text=_("Run Batch File:"), anchor=W, command=self.Set_Input_States_BATCH)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Preprocess_CRC = Label(gen_settings,text=_("Preprocess CRC Data"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Reduce_Memory = Label(gen_settings,text=_("Reduce Memory Use"))
        self.Label_Reduce_Memory.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Checkbutton_Reduce_Memory = Checkbutton(gen_settings,text=_("(needed for large designs or low memory computers)"), anchor=W)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Wait = Label(gen_settings,text=_("Wait for Laser to Finish"))
        self.Label_Wait.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Checkbutton_Wait = Checkbutton(gen_settings,text=_("(after all data has been sent over USB)"), anchor=W)
        ...
        self.Label_Inkscape_title = Label(gen_settings,text=_("Inkscape Options"))
        self.Label_Inkscape_title.place(x=xd_label_L, y=D_Yloc, width=gen_width-40, height=21)
        
        D_Yloc=D_Yloc+D_dY
        font_entry_width=215
        self.Label_Inkscape_Path = Label(gen_settings,text=_("Inkscape Executable"))
        ...
        self.Inkscape_Path = Button(gen_settings,text=_("Find Inkscape"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Ink_Timeout = Label(gen_settings,text=_("Inkscape Timeout"))
        self.Label_Ink_Timeout.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Label_Ink_Timeout_u = Label(gen_settings,text=_("minutes"), anchor=W)
        ...
        D_Yloc=D_Yloc+D_dY*.25
        self.Label_Inkscape_title2 = Label(gen_settings,text=_("Laser-M3 Options"))
        self.Label_Inkscape_title2.place(x=xd_label_L, y=D_Yloc, width=gen_width-40, height=21)
        
        D_Yloc=D_Yloc+D_dY
        self.Labelshow_power = Label(gen_settings,text=_("Show Power Settings"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Labelshow_test = Label(gen_settings,text=_("Show Test Fire Button"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Max_Power = Label(gen_settings,text=_("Maximum Power Setting"))
        self.Label_Max_Power.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        self.Label_Max_Power_u = Label(gen_settings,text=_("%   (Setting this too high may damage laser tube)"), anchor=W)
        ...
        self.Label_no_com = Label(gen_settings,text=_("Home in Upper Right"))
        ...
        D_Yloc=D_Yloc+D_dY 
        self.Label_Board_Name = Label(gen_settings,text=_("Board Name"), anchor=CENTER)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Area_Width = Label(gen_settings,text=_("Laser Area Width"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Area_Height = Label(gen_settings,text=_("Laser Area Height"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_X_Scale = Label(gen_settings,text=_("X Scale Factor"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Y_Scale = Label(gen_settings,text=_("Y Scale Factor"))
        ...
        self.Label_SaveConfig = Label(gen_settings,text=_("Configuration File"))
        self.Label_SaveConfig.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)

        self.GEN_SaveConfig = Button(gen_settings,text=_("Save"))
        ...
        self.GEN_Close = Button(gen_settings,text=_("Close"))
    ...
    def RASTER_Settings_Window(self):
        ...
        raster_settings.title(_("Raster Settings"))
        raster_settings.iconname(_("Raster Settings"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Rstep = Label(raster_settings,text=_("Scanline Step"), anchor=CENTER)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_EngraveUP = Label(raster_settings,text=_("Engrave Bottom Up"))
        ...
        ############
        D_Yloc=D_Yloc+D_dY 
        self.Label_Halftone_DPI = Label(raster_settings,text=_("Halftone Resolution"), anchor=CENTER)
...
##            trace_variable(self.unsharp_t, self.Entry_Unsharp_Threshold_Callback)        
        ...
        self.RASTER_Close = Button(raster_settings,text=_("Close"))
    ...
    def ROTARY_Settings_Window(self):
        ...
        rotary_settings.title(_("Rotary Settings"))
        rotary_settings.iconname(_("Rotary Settings"))
        ...
        self.Label_Rotary_Enable = Label(rotary_settings,text=_("Use Rotary Settings"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_R_Scale = Label(rotary_settings,text=_("Rotary Scale Factor (Y axis)"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Rapid_Feed = Label(rotary_settings,text=_("Rapid Speed (default=0)"))
        ...
        self.GEN_Close = Button(rotary_settings,text=_("Close"))
        self.GEN_Close.place(x=Xbut, y=Ybut, width=130, height=30, anchor="center")
        self.GEN_Close.bind("<ButtonRelease-1>", self.Close_Current_Window_Click)
    ...
    def TRACE_Settings_Window(self, dummy=None):
        ...
        trace_window.title(_("Trace Boundary"))
        trace_window.iconname(_("Trace Boundary"))
        ...
        self.Label_Trace_Power = Label(trace_window,text=_("Laser Power Level During Trace"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Trace_Gap = Label(trace_window,text=_("Gap Between Design and Trace"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Laser_Trace = Label(trace_window,text=_("Laser 'On' During Trace"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Trace_Button = Button(trace_window,text=_("Trace Boundary With Laser Head"),command=Close_and_Send_Click)
        ...
        self.Trace_Close = Button(trace_window,text=_("Cancel"),command=Close_Click)
    ...
    def EGV_Send_Window(self,EGV_filename):
        ...
        egv_send.title(_("EGV Send"))
        egv_send.iconname(_("EGV Send"))
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_Preprocess_CRC = Label(egv_send,text=_("Preprocess CRC Data"))
        self.Label_Preprocess_CRC.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        ...
        D_Yloc=D_Yloc+D_dY
        self.Label_N_EGV_Passes = Label(egv_send,text=_("Number of EGV Passes"))
        ...
        D_Yloc=D_Yloc+D_dY
        font_entry_width=215
        self.Label_Inkscape_Path = Label(egv_send,text=_("EGV File:"))
        self.Label_Inkscape_Path.place(x=xd_label_L, y=D_Yloc, width=w_label, height=21)
        ...
        self.EGV_Close = Button(egv_send,text=_("Cancel"))
        ...
        self.EGV_Send = Button(egv_send,text=_("Send EGV Data"),command=Close_and_Send_Click)
...
opts, args = None, None
try:
    opts, args = getopt.getopt(sys.argv[1:], "hpd o:",["help", "pi", "debug", "open="])
except:
...
if open_file and os.path.isfile(open_file):
    app.Load_Design_File(open_file)
